...
const MainAppButton = ({ size, isLoading, backgroundColor, width, children }: MainAppButtonProps) => {
    return (
        <Button variant='solid' bg={backgroundColor ?? "primaryColor.900"} color="appWhiteColor" isLoading={isLoading} width={width ?? "lg"} size={size ?? 'md'} loadingText='Please wait...'>{children}</Button>
    )
}
