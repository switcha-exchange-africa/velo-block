...
export const adsOrdersApi = baseApi.injectEndpoints({
    endpoints: (builder) => ({
        getBuyAds: builder.query<any, any>({
            query: ({arg, pageNumber}) => `${endpoints.P2P_BUY_ADS_URL}=${arg}&perpage=10&page=${pageNumber}`,
        ...
        getSellAdsUSDT: builder.query<any, any>({
            query: ({arg, pageNumber}) => `${endpoints.P2P_SELL_ADS_URL}=${arg}&perpage=10&page=${pageNumber}`,
            transformResponse: (responseData: any) => {
                return responseData;
            },
        }),
